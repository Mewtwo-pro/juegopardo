...
// Configuración de las imágenes
  const imgA = new Image(); imgA.src = '/assets/player/select.png';
  const imgB = new Image(); imgB.src = '/assets/player/selectb.png';

  const estado = {
    x1: -200,      // Posición inicial izquierda (fuera del canvas)
    x2: 800,       // Posición inicial derecha (fuera del canvas)
    targetX: 300,  // Posición final (donde se encontrarán)
    velocidad: 0.1 // Factor de suavizado (Easing)
  };

function animar() {
    ctx.clearRect(0, 0, canvas.width, canvas.height);

    // Aplicar interpolación lineal suavizada (Lerp)
    estado.x1 += (estado.targetX - estado.x1) * estado.velocidad;
    estado.x2 -= (estado.x2 - (estado.targetX + 100)) * estado.velocidad; // +100 para separar un poco las imágenes


    if (Math.abs(estado.targetX - estado.x1) > 0.1) {
      requestAnimationFrame(animar);
    }
  }

  // Iniciar cuando las imágenes carguen
  //Promise.all([imgA.decode(), imgB.decode()]).then(animar);
...
function cargarImgData(){
    ...
    playerFondo.src = "/assets/player/fondo.png";
...
function dibujarPersonaje() {
    ...
    // Dibujar
    ctx.drawImage(imgA, estado.x1, -150, 500, 500);
    ctx.drawImage(imgB, estado.x2, -150, 500, 500);
